...
static int reinitialize_bands_from_config(void)
{
    ...
    if (new_count < old_count) {
        ...
        log_printf("Config reload removed %d band(s).\n", old_count - new_count);
    }
    num_bands = new_count;
    ...
    if (config.fftplaneffort != fftplaneffort) {
        ...
        log_printf("Updated fftplaneffort to %d (new plans on next restart).\n", fftplaneffort);
    }
    ...
    for (int i = 0; i < new_count; i++) {
        ...
        if (b->samplerate != bc->samplerate ||
            ...
            log_printf("Band %d device/rate settings changed; restart required for full apply.\n", i);
        }
    ...
    if (new_count > old_count) {
        for (int i = old_count; i < new_count; i++) {
            ...
            if (band_init(b, 0) != 0) {
                log_printf("Config reload failed to initialize new band %d; keeping %d bands active.\n",
                           i, i);
                new_count = i;
                break;
            }
            ...
            if (alsa_open_device(b) != 0) {
                log_printf("Config reload failed to open device for new band %d; keeping %d bands active.\n",
                           i, i);
                if (b->started)
                    band_deinit(b);
                new_count = i;
...
static void regenerate_runtime_assets(void)
{
    ...
    for (int i = 0; i < config.num_bands; i++) {
        ...
        int rc = waterfall_write_band_pngs(tmpdir_pngs, i,
                                           ...
                                           b->maxzoom);
        if (rc != 0)
            log_printf("Failed to regenerate band PNGs for band %d\n", i);
    }

    if (band_write_bandinfo_js(config.public_dir) != 0)
        log_printf("Failed to regenerate bandinfo.js\n");
}
...
static void apply_config_reload(int how)
{
    ...
    if (parse_config(g_config_file) != 0) {
        log_printf("Config reload failed for '%s'\n", g_config_file);
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2 && strcmp(argv[1], "--test-band") == 0) {
        ...
        waterfall_init_palette();
        fprintf(stderr, "Generating band label PNG: center=%.1f kHz  bw=%.1f kHz  -> %s\n",
                center_khz, bw_khz, out);
        int rc = waterfall_write_bandlabel_png(out, center_khz, bw_khz, 1024);
        if (rc == 0)
            fprintf(stderr, "OK: %s written (1024x14, 4-color palette, indexed PNG)\n", out);
        else
    ...
    if (parse_config(g_config_file) != 0) {
        fprintf(stderr, "Failed to parse configuration '%s'. Exiting.\n", g_config_file);
        return 1;
    }
...

    {
        ...
        snprintf(rmcmd, sizeof(rmcmd), "rm -f %s/*", tmpdir);
        system(rmcmd);
        mkdir(tmpdir, 0755);
    }
    ...
    for (int i = 0; i < config.num_bands; i++) {
        ...
        if (band_init(b, 0) != 0) {
            log_printf("band_init failed for band %d\n", i);
            return 1;
        }
        ...
        if (config.public_dir[0]) {
            ...
            int rc = waterfall_write_band_pngs(tmpdir_pngs, i,
                                               ...
                                               b->maxzoom);
            if (rc != 0)
                fprintf(stderr, "Warning: failed to write band label PNGs for band %d\n", i);
        }
    }
    ...
    if (config.public_dir[0]) {
        if (band_write_bandinfo_js(config.public_dir) != 0)
            fprintf(stderr, "Warning: failed to write bandinfo.js\n");
    }
    ...
    for (int i = 0; i < config.num_bands; i++) {
        ...
        if (alsa_open_device(b) != 0) {
            fprintf(stderr, "Fatal: failed to open audio device for band %d. Exiting.\n", i);
            return 1;
        }
    ...
    while (!g_quit) {
        ...
        if (g_reload) {
            g_reload = 0;
            log_printf("Reloading configuration (SIGHUP)...\n");
        ...
        if (g_configreload) {
            ...
            g_configreload = 0;
            if (how == 3) {
                log_printf("Shutdown requested via ~~configreload.\n");
                g_quit = 1;
            } else {
                log_printf("Reloading configuration (how=%d)...\n", how);
