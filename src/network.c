    ...
    for (int i = 0; i < g_stationinfo_count; i++) {
        ...
        if (fabsf(e->freq_khz - freq_khz) < 0.0005f) {
            if (e->lines < 3 && (int)strlen(e->text) < 170) {
                strncat(e->text, "<br>", sizeof(e->text) - strlen(e->text) - 1);
...
{
    if (config.fakeusers >= 0)
        return config.fakeusers;
...
static void http_send_404(int idx, const char *body)
{
    HttpConn *c = &hp[idx];
    int close = (strstr(c->buf, "Connection: close") != NULL
              || strstr(c->buf, "connection: close") != NULL);
    http_send_response(idx, "404 Not Found", body);
    if (close) {
        hp_close(idx);
    } else {
        c->buf_len = 0;
...
static void http_sendfile(int idx, const char *mime, const char *filepath, int use_cache)
{
    HttpConn *c = &hp[idx];
    ...
    if (fd < 0) {
        log_printf("fd=%i -%s-: sending 404 for file not found: %s\n",
                   c->fd, c->ip_str, filepath);
        http_send_404(idx, "File not available.\r\n");
        return;
    }
    ...
    hlen += snprintf(hdr + hlen, sizeof(hdr) - (size_t)hlen,
        ...
        "Server: WebSDR/%s\r\n"
        "Content-Length: %lld\r\n",
        VERSION_STRING, (long long)st.st_size);
    if (mime)
        hlen += snprintf(hdr + hlen, sizeof(hdr) - (size_t)hlen,
            "Content-Type: %s\r\n", mime);
    hlen += snprintf(hdr + hlen, sizeof(hdr) - (size_t)hlen,
        "%s\r\n"
        "Connection: %s\r\n\r\n",
        use_cache ? "Cache-control: max-age=3600" : "Cache-control: no-cache",
        client_wants_close ? "close" : "keep-alive");

    if (socket_write_all(c->fd, hdr, (size_t)hlen) != 0) {
    ...
    while ((n = read(fd, buf, sizeof(buf))) > 0) {
        if (socket_write_all(c->fd, buf, (size_t)n) != 0) {
            hp_close_write_fail(idx);
            close(fd);
            return;
        }
    ...
    close(fd);
    g_stats.http_bytes += (long long)(hlen + st.st_size);
...
static void handle_tilde_stream(int idx)
{
    ...
    if (cl) {
        ...
        pthread_mutex_unlock(&worker_dsp_mutex);
        log_printf("fd=%i -%s-: assigned stream band=%i format=%i\n",
...
static void handle_tilde_waterstream(int idx)
{
    ...
    if (cl) {
        ...
        c->ws_audio_user = 0;
        pthread_mutex_unlock(&worker_dsp_mutex);
        log_printf("fd=%i -%s-: assigned waterstream band=%i fmt=%i\n",
                   c->fd, c->ip_str, cl->band_idx, fmt);
    }
...
static void http_sendfile_ssi(int idx, const char *filepath)
{
    HttpConn *c = &hp[idx];
    int body_len = 0;
    char *body = ssi_process_file(filepath, 0, &body_len);
    if (!body || body_len <= 0) {
        free(body);

        http_sendfile(idx, "text/html", filepath, 0);
        return;
    }
    ...
    if (socket_write_all(c->fd, body, (size_t)body_len) != 0) {
        free(body);
        hp_close_write_fail(idx);
        return;
    }
    g_stats.http_bytes += (long long)(hlen + body_len);
    free(body);
...
static void ws_upgrade(int idx, const char *key)
{
    ...
    log_printf("fd=%i -%s-: changing to WebSocket\n", c->fd, c->ip_str);
    ...
    if (cl) {
        ...
        pthread_mutex_unlock(&worker_dsp_mutex);
        log_printf("fd=%i -%s-: no free client slots\n", c->fd, c->ip_str);
        hp_close(idx);
    }
}
        ...
        if (eol) {
            *eol = '\0';
            if (!strstr(c->buf, "Host:")) {
                log_printf("fd=%i -%s-: Websocket upgrade failed, missing host\n",
                           c->fd, c->ip_str);
                hp_close(idx);
                return;
            }
...
void init_network(int port)
{
    ...
    if (listen_fd == -1) {
        if (errno == EAFNOSUPPORT)
            fprintf(stderr, "Your Linux kernel does not support IPv6.\n");
        else
    ...
    if (config.myhost[0]) {
        ...
        if (!he) {
            fprintf(stderr, "Can't find myhost \"%s\" - binding to all.\n", config.myhost);
    ...
    if (bind(listen_fd, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
        ...
        if (errno == EACCES && port < 1024)
            fprintf(stderr, "Listening on port %i failed - that needs root permissions.\n", port);
        else if (errno == EADDRINUSE)
            fprintf(stderr, "Listening on port %i failed - another program seems to use it already.\n", port);
        else
            perror("bind failed");
        exit(1);
    }
    ...
    if (listen(listen_fd, SOMAXCONN) != 0) { perror("listen failed"); exit(1); }
    ...
    log_printf("Network initialized, listening on port %d\n", port);
}
...
void process_network_events(void)
{
    ...
    for (int i = 0; i < nev; i++) {
        ...
        if ((tag & EPOLL_TAG_MASK) == EPOLL_TAG_LISTEN) {
            ...
            for (;;) {
                ...
                if (HTTP_MAX_CONN_PER_IP > 0 &&
                    ...
                    log_printf("fd=%i -%s-: HTTP connection rejected (per-ip cap %d)\n",
                ...
                for (int j = 0; j < HP_MAX; j++) {
                    if (hp[j].state && hp[j].fd == cfd) {
                        log_printf("fd=%i -%s-: Oops, old hp still open: %i %s\n",
                ...
                if (slot == -1) {
                    log_printf("fd=%i -%s-: Oops: too many http connections! Sending 500.\n",
                               cfd, ip);
                    write(cfd,
        ...
        if (room <= 0) {
            ...
            log_printf("fd=%i -%s-: HTTP buffer overflow, closing\n", c->fd, c->ip_str);
            hp_close(slot);
            continue;
        }
        ...
        if (n > 0) {
            ...
            log_printf("fd=%i -%s-: Disconnect on socket\n", c->fd, c->ip_str);
            hp_close(slot);
        }
    }
}
