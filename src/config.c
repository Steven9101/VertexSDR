...
int parse_config(const char *filename) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strcmp(key, "maxusers") == 0) {
        ...
        } else if (strcmp(key, "fakeusers") == 0) {
            config.fakeusers = atoi(val);
            ...
            if (strcmp(key, "centerfreq") == 0) {
                ...
                if (sscanf(val, "%255s %255[^\n]", arg1, arg2) == 2
                    ...
                    strncpy(current_band->device, arg2, sizeof(current_band->device)-1);
                    current_band->device_type = 0;
                } else {
                    strncpy(current_band->device, val, sizeof(current_band->device)-1);
                }
