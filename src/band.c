                                 ...
                                 int *fftlen, int *fftlen2, int *maxzoom)
{
    ...
    switch (v) {
        ...
        default:
            fprintf(stderr, "Sample rate %i is not recognized; continuing with fallback settings.\n",
                    samplerate);
            *fftlen  = 1536;
            *fftlen2 = 1024;
            *maxzoom = 1;
...
int band_init(struct BandState *b, int reinit)
{
    ...
    if (b->centerfreq_khz == -9999.0) {
        fprintf(stderr, "Band '%s' is missing centerfreq.\n", b->name);
    ...
    if (samplerate == -9999 || samplerate == 0) {
        fprintf(stderr, "Band '%s' is missing samplerate; defaulting to 48000 Hz.\n", b->name);
        samplerate = 48000;
    ...
    if (b->extrazoom > 0) {
        if (b->extrazoom > 5) {
            fprintf(stderr, "Band '%s': extrazoom=%i is out of range; resetting to 0.\n",
                    b->name, b->extrazoom);
            b->extrazoom = 0;
        } else if (b->extrazoom + maxzoom > 7) {
            fprintf(stderr, "Band '%s': extrazoom=%i exceeds the zoom budget; resetting to 0.\n",
                    b->name, b->extrazoom);
            b->extrazoom = 0;
        } else {
...
fftlen_set:
    ...
    for (int z = 0; z <= b->maxzoom && z < WF_MAX_ZOOMS; z++) {
        ...
        if (!b->wf_zoom_row[z] || !b->wf_zoom_power[z]) {
            fprintf(stderr, "Band %d: failed to allocate waterfall zoom pyramid.\n", b->band_index);
            return -1;
        }
    }
    ...
    if (vk_waterfall_prepare_band(b) == 0)
        fprintf(stderr, "Band %d: Vulkan waterfall path enabled.\n", b->band_index);
#endif

    unsigned int fflags = fftw_flags_from_effort(fftplaneffort);

    fprintf(stderr, "Band %i: planning FFTs...", b->band_index);
    fflush(stderr);
...
int band_write_bandinfo_js(const char *pubdir)
{
    ...
    fclose(f);
    fprintf(stderr, "Band frequency axis images written.\n");
    return 0;
}
...
int band_load_balance(struct BandState *b, const char *filename)
{
    ...
    if (!f) {
        fprintf(stderr, "Balance file '%s' could not be opened: %s\n",
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%f %f %f", &freq_norm, &gain_re, &gain_im) != 3) {
            fprintf(stderr, "Balance file parse error: %s", line);
            continue;
        }
...
int band_load_equalize(struct BandState *b, const char *filename)
{
    FILE *f = fopen(filename, "r");
    if (!f) {
        fprintf(stderr, "Equalizer file '%s' could not be opened: %s\n",
                filename, strerror(errno));
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (sscanf(line, "%f %f", &freq_hz, &gain_db) != 2) {
            fprintf(stderr, "Equalizer file parse error: %s", line);
            continue;
        }
