...
static char log_prev_filename[512] = "";
...
void logging_init(void)
{
    ...
    if (g_config.logfileinterval > 0) {
        ...
        if (!f) {
            fprintf(stderr, "Log file unavailable for append: %s\n", log_filename);
            fflush(stderr);
        } else {
            log_stream = f;
        }
...
void logging_rotate(void)
{
    ...
    if (!log_stream) {
        fprintf(stderr, "Log rotation failed: unable to reopen %s\n", log_filename);
        fflush(stderr);
    ...
    if (old) {
        ...
        if (rc != 0) {
            fprintf(stderr, "Log compression thread could not be started (rc=%d).\n", rc);
            free(old);
        } else {
            pthread_detach(th);
        }
...
void log_printf_dsp(const char *fmt, ...)
{
    ...
    if (n < 0) {
        fprintf(stderr, "DSP log buffer formatting failed.\n");
        return;
    }
...
void log_flush_dsp(void)
{
    ...
    if (len == DSP_LOGBUF_SIZE - 1) {
        log_printf("DSP log buffer filled to %d bytes; output was truncated.\n", len);
    }
    log_printf("2%s", tmp);
}
