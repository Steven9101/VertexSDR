...
static int create_vkfft_plan(VkFFTBandPlan *p, size_t fft_len, int r2c, int inverse)
{
    ...
    if (res != VKFFT_SUCCESS) {
        fprintf(stderr, "VkFFT plan initialization failed (err=%d, len=%zu, r2c=%d, inv=%d).\n",
...
int fft_backend_init_global(void)
{
...
#ifndef USE_VULKAN
    fprintf(stderr, "vkfft was requested, but Vulkan support is disabled. Falling back to fftw.\n");
    return 0;
#else
    if (!vk_waterfall_is_ready()) {
        fprintf(stderr, "vkfft is unavailable because Vulkan is not initialized. Falling back to fftw.\n");
        return 0;
    }
    ...
    if (vkCreateFence(g_vk_dev, &fci, NULL, &g_vk_fence) != VK_SUCCESS) {
        fprintf(stderr, "vkfft fence creation failed. Falling back to fftw.\n");
        return 0;
    }
    ...
    fprintf(stderr, "vkfft backend enabled on the shared Vulkan device.\n");
    return 0;
#endif
}
...
int fft_backend_prepare_demod_ifft(struct DemodTable *dt, int fft_size, int c2r)
{
    ...
    if (res != VKFFT_SUCCESS) {
        fprintf(stderr, "VkFFT demod IFFT initialization failed (err=%d, size=%d, c2r=%d).\n",
