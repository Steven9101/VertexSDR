...
static int parse_ws_frame(struct Client *c, const uint8_t *buf, int len)
{
    ...
    if (plen == 126) { if (len < 4) return 0; plen = (buf[2] << 8) | buf[3]; hdrlen = 4; }
        ...
        if (len < 10) return 0;
        uint64_t p64 = ((uint64_t)buf[2]<<56)|((uint64_t)buf[3]<<48)|
                       ((uint64_t)buf[4]<<40)|((uint64_t)buf[5]<<32)|
                       ((uint64_t)buf[6]<<24)|((uint64_t)buf[7]<<16)|
                       ((uint64_t)buf[8]<<8) | (uint64_t)buf[9];
        if (p64 > (uint64_t)(CLIENT_BUF_SIZE - 1)) { client_close(c); return -1; }
        plen = (int)p64;
        hdrlen = 10;
    }
    if (mask) hdrlen += 4;
...
static void cmd_param(struct Client *c, const char *qs)
{
    ...
    if (band >= 0 && band < config.num_bands && band != c->band_idx) {
        ...
        log_printf("fd=%d: /~~param switched to band %d\n", c->fd, band);
...
static void cmd_waterparam(struct Client *c, const char *qs)
{
    ...
    if (band >= 0 && band < config.num_bands && band != c->band_idx) {
        ...
        bands[band].audio_clients++;
        log_printf("fd=%d: /~~waterparam switched to band %d\n", c->fd, band);
        client_reset_audio_state_on_band_change(c);
        c->uu_chseq = chat_get_pending_chseq();
        c->audio_seqno = 0;
        c->wf_prev_valid = 0;
                                  ...
                                  float *audio_f, float scale, int smeter_raw)
{
    ...
    for (int i = 0; i < 128; i++) {
        ...
        if (c->conv_type & 0x10)
            ...
            c->pred_accum = old_accum + ((16 * predicted_sample) >> 3);
    }
...
void client_dispatch_audio(struct BandState *b, int phase_flag)
{
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (phase_flag) {
            double tune_offset_norm = (c->tune_khz - b->centerfreq_khz)
                                    / ((double)sr / 1000.0);
            double tn = 0.0;
    ...
    if (g_config.idletimeout > 0) {
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (now - c2->last_active > g_config.idletimeout) {
                log_printf("fd=%i -%s-: WebSocket client idle timeout reached (%i s)\n",
