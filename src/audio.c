...
int alsa_open_device(struct BandState *b)
{
    ...
    if (!dev || dev[0] == '\0') {
        fprintf(stderr, "Band %i: audio device name is empty.\n", b->band_index);
        return -1;
    }
    ...
    for (int i = 0; i < g_alsa_handle_count; i++) {
        if (strncmp(g_alsa_handles[i].dev_name, dev, 255) == 0) {
            ...
            b->fd = pfds[0].fd;
            fprintf(stderr, "Band %i: reusing shared ALSA capture handle '%s' (channel %i).\n",
    ...
    if (err < 0) {
        fprintf(stderr, "ALSA open failed for '%s': %s.\n",
    ...
    if (err < 0) {
        fprintf(stderr, "ALSA sample-rate setup failed for '%s' (%u Hz): %s.\n",
    ...
    if (err < 0) {
        fprintf(stderr, "ALSA channel setup failed for '%s' (%u channels): %s.\n",
...
hw_err:
    fprintf(stderr, "ALSA hardware parameter setup failed for '%s': %s.\n",
            dev, snd_strerror(err));
    snd_pcm_hw_params_free(hwp);
    exit(1);

sw_err:
    fprintf(stderr, "ALSA software parameter setup failed for '%s': %s.\n",
            dev, snd_strerror(err));
    snd_pcm_sw_params_free(swp);
    exit(1);
}
...
int rtltcp_open_device(struct BandState *b)
{
    ...
    if (!sp) { fprintf(stderr, "Band %i: invalid rtl_tcp device spec '%s'.\n", b->band_index, spec); return -1; }
    sp++;
    ...
    if (getaddrinfo(host, portstr, &hints, &res) != 0) {
        fprintf(stderr, "Band %i: unable to resolve rtl_tcp host '%s'.\n", b->band_index, host);
        return -1;
    }

    int fd = socket(res->ai_family, SOCK_STREAM, 0);
    if (fd < 0) {
        fprintf(stderr, "Band %i: rtl_tcp socket creation failed: %s\n",
    ...
    if (connect(fd, res->ai_addr, res->ai_addrlen) < 0) {
        fprintf(stderr, "Band %i: rtl_tcp connection to %s:%d failed: %s\n"
                        "         Start rtl_tcp first, for example: rtl_tcp -a 127.0.0.1\n",
    ...
    while (got < 12) {
        ...
        if (n <= 0) { fprintf(stderr, "Band %i: rtl_tcp greeting read failed.\n", b->band_index); close(fd); return -1; }
        got += n;
    }
    if (memcmp(magic, "RTL0", 4) != 0)
        fprintf(stderr, "Band %i: rtl_tcp greeting did not start with RTL0 (got %02x%02x%02x%02x).\n",
    ...
    b->fd = fd;
    b->rtltcp_fd = fd;
    fprintf(stderr, "Band %i: rtl_tcp input is ready.\n", b->band_index);
    return 0;
}
...
int rtltcp_read_samples(struct BandState *b)
{
    ...
    if (n <= 0) {
        ...
        fprintf(stderr, "Band %i: rtl_tcp read failed: %s\n", b->band_index, strerror(errno));
        return -1;
    }
...
static void *stdin_reader_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (n <= 0) {
            stdin_eof = 1;
            uint64_t v = 1;
            if (write(stdin_event_fd, &v, sizeof(v)) < 0) {}
            break;
        }
...
static int stdin_process_samples(struct BandState *b)
{
    ...
    if (total_consumed < avail) {
        uint64_t kick = 1;
        if (write(stdin_event_fd, &kick, sizeof(kick)) < 0) {}
    }
...
int stdin_open_device(struct BandState *b)
{
    if (stdin_parse_format(b->stdinformat_str, b->noniq,
        ...
        fprintf(stderr, "Band %i: invalid stdin sample format '%s'.\n",
                b->band_index,
    ...
    if (stdin_event_fd < 0) {
        fprintf(stderr, "stdin eventfd creation failed: %s\n", strerror(errno));
        return -1;
    }
    ...
    if (pthread_create(&stdin_reader_tid, NULL, stdin_reader_thread, NULL) != 0) {
        fprintf(stderr, "Failed to start stdin reader thread: %s\n", strerror(errno));
    ...
    if (pthread_create(&stdin_dsp_tid, NULL, stdin_dsp_thread, b) != 0) {
        fprintf(stderr, "Failed to start stdin DSP thread: %s\n", strerror(errno));
    ...
    fprintf(stderr, "Band %i: reading %s %s samples from stdin using the threaded path.\n",
            b->band_index, stdin_format_name, stdin_is_iq ? "IQ" : "real");
    fprintf(stderr, "Band %i: stdin DSP processing is running off the network thread.\n",
            b->band_index);
    return 0;
}
...
{
    uint64_t v;
    if (read(stdin_event_fd, &v, sizeof(v)) < 0) {}
...
int alsa_read_samples(struct BandState *b)
{
    ...
    if (n < 0) {
        ...
        if (n < 0) {
            fprintf(stderr, "Band %i: ALSA capture read failed: %s\n",
