...
int vk_waterfall_init_global(void)
{
    ...
    if (vkCreateInstance(&ici, NULL, &g_instance) != VK_SUCCESS) {
        fprintf(stderr, "vk_waterfall: cannot create Vulkan instance\n");
        return -1;
    }
    ...
    if (dc == 0) {
        fprintf(stderr, "vk_waterfall: no Vulkan devices\n");
        return -1;
    }
    ...
    if (g_queue_family == UINT32_MAX) {
        fprintf(stderr, "vk_waterfall: no compute queue\n");
        return -1;
    }

    VkPhysicalDeviceProperties props;
    vkGetPhysicalDeviceProperties(g_pdev, &props);
    fprintf(stderr, "vk_waterfall: using %s\n", props.deviceName);
    ...
    if (vkCreateDevice(g_pdev, &dci, NULL, &g_dev) != VK_SUCCESS) {
        fprintf(stderr, "vk_waterfall: device creation failed\n");
        return -1;
    }
    ...
    g_vk_ready = 1;
    fprintf(stderr, "vk_waterfall: initialized\n");
    return 0;
}
...
int vk_waterfall_prepare_band(struct BandState *b)
{
    ...
    b->vk_wf = vb;
    fprintf(stderr, "vk_waterfall: band prepared (top_px=%d, %d levels)\n",
            top_px, vb->n_levels);
    return 0;
