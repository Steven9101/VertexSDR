                            ...
                            struct Client *c, unsigned int peak_bin)
{
    ...
    if (c->notch_tracked_bin == a4) {
        int v = c->notch_consec_count;
        if (v <= 4) c->notch_consec_count = v + 1;
    } else {
        c->notch_consec_count = 0;
        c->notch_tracked_bin  = a4;
    }
    ...
    if (a4 == result) {
        ...
        int v5 = c->notch_hold_counter;
        if (v5) {
            c->notch_hold_counter = v5 - 1;
        } else {
            c->notch_confirmed_bin = 0;
            if (c->notch_consec_count <= 4) return;
            c->notch_hold_counter  = 10;
            c->notch_confirmed_bin = a4;
            result = a4;
        }
    }
...
static void demod_ssb_core(struct BandState *b, int tune_bin,
                           ...
                           struct DemodTable *dt, int phase_flag, int a6)
{
                          ...
                          struct DemodTable *dt, int phase_flag, int a6)
{
    ...
    int v7 = 1 - half;
    if (half > v7) {
        int src_bin = v7 + tune_bin;
        float *filt = filter_buf + 512 + v7;

        for (int k = v7; k < half; k++) {
            int dst = k & mask;
                     ...
                     struct DemodTable *dt, int phase_flag)
{
    ...
    if (fm_half > v6) {
        int src_bin = v6 + tune_bin;
        float *filt = filter_buf + 512 + v6;

        for (int k = v6; k < fm_half; k++) {
            if (src_bin >= 0 && src_bin < N) {
    ...
    if (c) {
        int v35 = c->filter_hi_bin - c->filter_lo_bin;
        double v36;
        if (c->band_ptr) {
            v36 = -0.0000367 * ((double)v35 * 0.03125) * ((double)v35 * 0.03125)
                + (double)v35 * 0.03125 * 0.001835
                - 0.0064
                - 0.0015;
        } else {
            v36 = 0.0125;
        ...
        c->demod_fm_sq = (var_per_sample > v36) ? 1 : 0;
    }
}
    ...
    if (half > v7) {
        for (int k = v7; k < half; k++) {
            ...
            if (v8 >= 0 && v8 < N) {
                ...
                din[dst][0] = spec[v8][0] * fw;
                din[dst][1] = spec[v8][1] * fw;
            } else {
                din[dst][0] = 0.0f;
                din[dst][1] = 0.0f;
            }
...
void compute_passband_filter(struct Client *c, struct BandState *b)
{
    ...
    double v4 = fmax(fabs(hi_khz), fabs(lo_khz));
    ...
    double v8 = fftlen_d * lo_khz / sr_d * 1000.0;
    int v10 = (v8 <= 0.0) ? (int)(v8 - 0.5) : (int)(v8 + 0.5);

    double v11 = fftlen_d * hi_khz / sr_d * 1000.0;
    int v13 = (v11 <= 0.0) ? (int)(v11 - 0.5) : (int)(v11 + 0.5);

    if (v10 == c->filter_lo_bin && v13 == c->filter_hi_bin)
        return;

    int v14 = 5122 - v10;
    int v15 = 1024 - v13;

    if (v10 > v13 || v14 > 6143 || v15 < 0) {
    ...
    if (v13 >= filter_half - 1)
        v13 = filter_half - 2;
    int v20 = 2 - filter_half;
    if (v10 >= v20)
        v20 = v10;
    ...
    int v25 = 2;
    if (v4 >= 1.0) v25 = (v4 < 2.7) ? 1 : 0;
    if (c->carrier_freq > 9000.0) v25 = 3;
    if (v13 * v20 > 0 && c->demod_mode != DEMOD_AM && c->demod_mode != DEMOD_AMSYNC)
        v25 |= 0x10;
    if (c->conv_type != v25) {
        c->conv_type = v25;
        c->conv_changed = 1;
    ...
    double v26 = (double)(v13 - v20);
    if (v13 * v20 < 0) {
        int v31 = -v20;
        v26 = (v13 <= v31) ? (double)v31 : (double)v13;
    }
    c->squelch_threshold = (float)(
        ...
        + 0.0504
        - 0.05
        - 0.09 * (0.00390625 * v26) * (0.00390625 * v26) *
                  (0.00390625 * v26) * (0.00390625 * v26)
    );
}
    ...
    if (!b->noniq) {
        ...
        if (phase) {
            ...
            if (half > 1) {
                ...
                float *v11 = (float *)(r2c + half + 1);
                float *v12 = (float *)(r2c + half - 2);
                float *v14 = (float *)(cx + N - 1);
                float *v15 = (float *)(phase + N - 1);
                float *v6_ptr = (float *)(cx + 1);
                float *v5_ptr = (float *)(phase + 1);

                for (int k = 1; k < half; k++) {
                    float v16 = v14[1];
                    float v17 = v15[1];

                    v11[0] = v6_ptr[0] - (v14[0] * v15[0] + v16 * v17);

                    float v18 = v16 * v15[0];
                    v11[1] = v6_ptr[1] - (v17 * v14[0] - v18);

                    float v20 = v6_ptr[1];
                    float v21 = v5_ptr[1];
                    v12[0] = v14[0] - (v6_ptr[0] * v5_ptr[0] + v20 * v21);

                    float v22 = v20 * v5_ptr[0];
                    v12[1] = v14[1] - (v21 * v6_ptr[0] - v22);
...
{
    int v3 = b->half_fftlen;
    int v4 = b->fftlen_real;
    ...
    if (v3 == v4 / 4)
        b->half_fftlen = 3 * v4 / 4;
    else
        b->half_fftlen = v4;

    int phase_flag = (b->sample_count != v4 / 4) ? 1 : 0;
