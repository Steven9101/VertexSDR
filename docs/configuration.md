...
Overrides displayed user count. `-1` disables.
...
VertexSDR also registers independently on **sdr-list.xyz** at the same time. Both directories receive per-band receiver data. You do not need separate configuration for sdr-list.xyz.

### `noorgserver`

Disables upstream directory registration.
